...
def adaptive_chunk(elements: List[Dict], chunk_size: int = 1200, overlap: int = 120) -> List[Dict]:
    ...
    for el in elements:
        ...
        if buffer_len + tokens > chunk_size and buffer:
            merged = '\n'.join(buffer)
            chunks.append({'content': merged, 'hash': hash_text(merged), 'source': el.get('source'), 'meta': {}})
            # overlap
            if overlap>0:
    ...
    if buffer:
        merged = '\n'.join(buffer)
        chunks.append({'content': merged, 'hash': hash_text(merged), 'source': elements[-1].get('source') if elements else '', 'meta': {}})
    return chunks
