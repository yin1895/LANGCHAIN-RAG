...
async def ingest(settings = Depends(get_settings)):
    with span('api_ingest', logger):
        ...
        store: FaissStore = GLOBAL.get('store') or FaissStore(settings.vector_store_path, settings.metadata_store_path, dim=None)
...
async def ask(req: AskRequest, settings = Depends(get_settings)):
    with span('api_ask', logger, top_k=req.top_k):
        ...
        logger.info(f"ask q_len={len(req.question)} hits={len(docs)} top_score={(docs[0]['score'] if docs else None)}")
