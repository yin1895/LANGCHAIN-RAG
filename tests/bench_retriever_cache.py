...
def bench(retriever, query, runs=50):
    ...
    ts = sorted(ts)
    return {'min_ms': ts[0], 'p50_ms': ts[len(ts)//2], 'p95_ms': ts[int(len(ts)*0.95)-1], 'mean_ms': sum(ts)/len(ts)}
...
def main():
    ...
    print('\nSummary:')
    print('vector-only no-cache mean(ms):', res0['mean_ms'], 'with-cache mean(ms):', res1['mean_ms'])
