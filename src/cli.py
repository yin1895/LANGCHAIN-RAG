...
from .rag.llm import BaseLLM, get_default_llm
...
def cmd_ingest(args) -> int:
    ...
    if args.limit_files:
        raw = ingest_files(
            files, max_pdf_mb=settings.max_pdf_mb, low_pdf_text_ratio=settings.low_pdf_text_ratio
        )
    else:
        raw = ingest_to_raw(
            str(root_path),
            max_pdf_mb=settings.max_pdf_mb,
            low_pdf_text_ratio=settings.low_pdf_text_ratio,
        )
    ...
    try:
        ...
        if para_total:
            print(
                f"[INGEST][STAT] 段落: total={para_total} docx={docx_paras} pdf={pdf_paras} math={math_paras} placeholder={placeholder_paras}"
            )
        if table_total:
    ...
    if getattr(args, "sample", 0):
        ...
        for i, r in enumerate(raw[: args.sample]):
            ...
            print(
                f"  #{i} type={r.get('type')} len={len(r.get('text','')) if 'text' in r else 0} {preview}"
            )
        # Count error types
    ...
    print(
        f"[INGEST] 分块数量: {len(chunks)} (chunk_size={settings.chunk_size}, overlap={settings.chunk_overlap})"
    )
...
def format_contexts(ctxs: List[Dict], limit: int = 220) -> str:
    ...
    for i, c in enumerate(ctxs, 1):
        ...
        lines.append(
            f"[ref {i} | score={c.get('score'):.3f}] {c.get('source')}\n  {snippet.replace('\n',' ')}"
        )
    return "\n".join(lines)
...
async def async_answer(
    question: str, top_k: int, show_ctx: bool, json_out: bool, bm25_weight: float
):
    settings = get_settings()
    embed = OllamaEmbeddings(settings.embed_model)
    store = FaissStore(settings.vector_store_path, settings.metadata_store_path, dim=None)
    ...
    if json_out:
        payload = {
            ...
            "answer": answer,
            "contexts": [
                {"ref": i + 1, "score": d["score"], "source": d["source"], "hash": d["hash"]}
...
def cmd_ask(args) -> int:
    ...
    for idx, q in enumerate(questions, 1):
        ...
        exit_code = asyncio.run(
            async_answer(q, args.top_k, args.show_context, args.json, args.bm25_weight)
        )
        if exit_code != 0:
            break
    return exit_code
...
def cmd_repl(args) -> int:
    ...
    while True:
        ...
        if args.show_context:
            ...
            print("\n片段:")
            pprint(
                [{"score": d["score"], "source": d["source"][:80], "hash": d["hash"]} for d in docs]
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pin.add_argument(
        "--limit-files", type=int, default=0, help="仅处理前N个docx文件用于调试或快速构建索引"
    )
