...
def copy_rows(src: Path, dst: Path):
    ...
    try:
        # users
        rows = s.execute(
            "SELECT id, username, password_hash, is_admin, is_active, created_at FROM users"
        ).fetchall()
        for r in rows:
            d.execute(
                "INSERT OR IGNORE INTO users (id, username, password_hash, is_admin, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                (
                    r["id"],
                    r["username"],
                    r["password_hash"],
        ...
        for r in rows:
            d.execute(
                "INSERT OR IGNORE INTO revoked_tokens (id, jti, revoked_at, revoked_by) VALUES (?, ?, ?, ?)",
        ...
        for r in rows:
            d.execute(
                "INSERT OR IGNORE INTO admin_audit (id, actor, action, target, created_at) VALUES (?, ?, ?, ?, ?)",
