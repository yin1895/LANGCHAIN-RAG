...
def paragraph_with_math(p) -> str:
    ...
    pattern_math = re.compile(r'<m:oMath[^>]*>.*?</m:oMath>|<m:oMathPara[^>]*>.*?</m:oMathPara>', re.DOTALL)
...
def extract_paragraphs(doc: Document) -> Iterable[Dict]:
    for p in doc.paragraphs:
        if not (p.text or has_math_xml(p._p.xml)):
            continue
        yield {'type': 'paragraph', 'text': paragraph_with_math(p), 'style': p.style.name if p.style else ''}
...
def _load_pdf(path: Path, max_mb: int = 25, low_ratio: float = 0.02) -> List[Dict]:
    ...
    if size_bytes > max_mb * 1024 * 1024:
        return [{'type':'error','source':str(path),'error':f'pdf_oversize>{max_mb}MB','size_bytes':size_bytes}]
    output = StringIO()
    try:
        ...
        if ratio < low_ratio:
            # 可能是扫描版或文本极少
            return [{'type':'error','source':str(path),'error':'pdf_low_text_ratio','size_bytes':size_bytes,'text_bytes':text_bytes,'ratio':ratio}]
    ...
    for i, block in enumerate(re.split(r'\n\s*\n', text)):
        ...
        if not blk:
            continue
        parts.append({'type':'paragraph','text':blk,'source':str(path),'order':i,'origin':'pdf'})
    return parts
...
def ingest_files(files: List[Path], max_pdf_mb: int = 25, low_pdf_text_ratio: float = 0.02) -> List[Dict]:
    all_items: List[Dict] = []
    for f in files:
        if f.suffix.lower()=='.docx':
            try:
